...
def build_audit_report_csv(payload: dict[str, object], report_sha256: str) -> bytes:
    ...
    columns = [
        ...
        "reporting_basis",
        "dcarbn_kg_co2e",
        "dcarbn_methodology_version",
        "uk_government_kg_co2e",
        "uk_government_methodology_version",
    ...
    for row in sorted(_mappings(payload.get("results")), key=lambda item: str(item.get("id", ""))):
        ...
        writer.writerow(
            [
                ...
                readiness.get("claim_wording"),
                row.get("id"),
                row.get("activity_id"),
                row.get("activity_date"),
                row.get("description"),
                row.get("scope"),
                row.get("scope_2_method"),
                row.get("scope_3_category"),
                row.get("original_activity_value"),
                row.get("original_activity_unit"),
                row.get("selected_factor_id"),
                row.get("factor_value"),
                row.get("factor_activity_unit"),
                row.get("gross_kg_co2e"),
                row.get("allocated_kg_co2e"),
                ...
                comparison.get("reporting_basis"),
                dcarbn.get("allocated_kg_co2e"),
...
def build_audit_report_pdf(payload: dict[str, object], report_sha256: str) -> bytes:
    ...
    doc = SimpleDocTemplate(
        ...
        subject="Governed greenhouse gas inventory report",
    )
    styles = _styles()
    story: list[object] = []
    ...
    story.extend(
        [
            Paragraph(
                escape(str(inventory.get("name", "Corporate GHG Inventory"))),
                styles["title"],
            ),
            Paragraph(
            ...
            Paragraph(
                f"{escape(claim_wording)}. Independent verification requires a separate assurance opinion.",
                styles["intro"],
            ),
            _section("Assurance readiness", styles),
            _field_table(
                [
                    ("Readiness status", readiness.get("status", "not_assessed")),
                    (
                        "External assurance required",
                        readiness.get("external_assurance_required", True),
                    ),
                ],
                styles,
            ),
            ...
            _field_table(
                [
                    ...
                    (
                        ...
                        _amount(totals.get("scope_2_market_based_kg_co2e"), "kg CO2e"),
                    ),
            ...
            Paragraph(
                escape(str(defence.get("preparation_basis", "Not reported"))),
                styles["body"],
            ),
            Spacer(1, 1.5 * mm),
            Paragraph(
                escape(str(defence.get("assurance_limitation", "Not reported"))),
                styles["body"],
            ),
            Spacer(1, 2 * mm),
            _field_table(
    ...
    if readiness_blockers:
        story.append(Paragraph("Outstanding validation controls", styles["body"]))
        ...
        [
            ...
            _field_table(
                [
                    ...
                    (
                        "Evidence coverage",
                        _amount(quality.get("evidence_coverage_percent"), "%"),
                    ),
                    (
                        "Quality distribution",
                        _quality_distribution(quality.get("level_distribution")),
                    ),
                ],
                styles,
            ),
    ...
    scope3_rows: list[list[object]] = [
        [Paragraph("Scope 3 inclusions and exclusions", styles["section_text"])]
    ]
    for item in dispositions:
        ...
        scope3_rows.append(
            [
                Paragraph(
                    f"<b>Category {category}: {disposition}</b><br/>{rationale}",
                    styles["table_body"],
                )
            ]
        )
