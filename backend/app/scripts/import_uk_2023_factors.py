...
from app.services.emission_factors import import_uk_2023_factor_workbook


def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Import the official final UK Government 2023 flat-format factor workbook."
    )
    parser.add_argument("workbook", type=Path)
    parser.add_argument("--dataset-version", default="2023-v1.1")
    parser.add_argument("--reporting-year", type=int, default=2023)
...
async def run() -> None:
    ...
    metadata = FactorSetImportMetadata(
        ...
        notes=(
            "UK Government 2023 final factor set, v1.1 June 2023 correction. "
            "HVO is available only when explicitly selected and evidenced."
        ),
    )
